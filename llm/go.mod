module github.com/PureMature/starport/llm

go 1.18

require github.com/PureMature/starport/base v0.0.3

require (
	github.com/1set/starlet v0.1.2-0.20240528100323-c91e367021b8 // indirect
	github.com/1set/starlight v0.1.1 // indirect
	github.com/chzyer/readline v0.0.0-20180603132655-2972be24d48e // indirect
	github.com/google/uuid v1.6.0 // indirect
	github.com/h2so5/here v0.0.0-20200815043652-5e14eb691fae // indirect
	go.starlark.net v0.0.0-20240123142251-f86470692795 // indirect
	go.uber.org/atomic v1.11.0 // indirect
	go.uber.org/multierr v1.9.0 // indirect
	go.uber.org/zap v1.24.0 // indirect
	golang.org/x/sys v0.0.0-20220715151400-c0bba94af5f8 // indirect
)
